...
// Purpose: 
//-----------------------------------------------------------------------------
CMSSGameMovement::CMSSGameMovement()
{
}
...
// Output : int
//-----------------------------------------------------------------------------
int CMSSGameMovement::GetCheckInterval( IntervalType_t type )
{
	// MSS ladders need to check every frame!!!
	if ( type == LADDER )
		return 1;
...
//-----------------------------------------------------------------------------
bool CMSSGameMovement::IsForceMoveActive()
{
...
//-----------------------------------------------------------------------------
void CMSSGameMovement::SwallowUseKey()
{
	mv->m_nOldButtons |= IN_USE;
	player->m_afButtonPressed &= ~IN_USE;

	GetMSSPlayer()->m_bPlayUseDenySound = false;
}
...
//			*ladder - 
//-----------------------------------------------------------------------------
void CMSSGameMovement::StartForcedMove( bool mounting, float transit_speed, const Vector& goalpos, CFuncLadder *ladder )
{
	LadderMove_t* lm = GetLadderMove();
	Assert( lm );
	...
	if ( ladder )
	{
		ladder->PlayerGotOn( GetMSSPlayer() );
...
//-----------------------------------------------------------------------------
bool CMSSGameMovement::ContinueForcedMove()
{
	LadderMove_t* lm = GetLadderMove();
	Assert( lm );
...
// Input  : &trace - ignored
//-----------------------------------------------------------------------------
bool CMSSGameMovement::OnLadder( trace_t &trace )
{
...
//			ladderOrigin - 
//-----------------------------------------------------------------------------
void CMSSGameMovement::Findladder( float maxdist, CFuncLadder **ppLadder, Vector& ladderOrigin, const CFuncLadder *skipLadder )
{
...
void CMSSGameMovement::GetSortedDismountNodeList( const Vector &org, float radius, CFuncLadder *ladder, CUtlRBTree< NearbyDismount_t, int >& list )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMSSGameMovement::ExitLadderViaDismountNode( CFuncLadder *ladder, bool strict, bool useAlternate )
{
...
// Input  : bOnLadder - 
//-----------------------------------------------------------------------------
void CMSSGameMovement::FullLadderMove()
{
#if !defined( CLIENT_DLL )
...
bool CMSSGameMovement::CheckLadderAutoMountEndPoint( CFuncLadder *ladder, const Vector& bestOrigin )
{
...
bool CMSSGameMovement::CheckLadderAutoMountCone( CFuncLadder *ladder, const Vector& bestOrigin, float maxAngleDelta, float maxDistToLadder )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMSSGameMovement::LookingAtLadder( CFuncLadder *ladder )
{
	if ( !ladder )
	{
		return false;
...
// Input  : &trace - 
//-----------------------------------------------------------------------------
bool CMSSGameMovement::CheckLadderAutoMount( CFuncLadder *ladder, const Vector& bestOrigin )
{
#if !defined( CLIENT_DLL )
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CMSSGameMovement::LadderMove( void )
{
...
/*
	...
	{
		GetMSSPlayer()->DisplayLadderHudHint();
	}
#endif
*/
...
void CMSSGameMovement::SetGroundEntity( trace_t *pm )
{
//BOXBOX removing
...
bool CMSSGameMovement::CanAccelerate()
{
	...
	static CMSSGameMovement g_GameMovement;
