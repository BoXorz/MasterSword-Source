...
//	#include "team.h" // BOXBOX removing teams
	#include "voice_gamemgr.h"
	#include "MSS_gameinterface.h"
	#include "MSS_cvars.h"

#ifdef DEBUG	
	#include "MSS_bot_temp.h"
#endif
...
//ConVar sv_hl2mp_item_respawn_time( "sv_hl2mp_item_respawn_time", "30", FCVAR_GAMEDLL | FCVAR_NOTIFY ); // BOXBOX Items don't spawn/respawn in MSS
//ConVar sv_report_client_settings("sv_report_client_settings", "0", FCVAR_GAMEDLL | FCVAR_NOTIFY );

//extern ConVar mp_chattime; // BOXBOX don't need this
...
static MSSViewVectors g_MSSViewVectors(
...
{
	return &g_MSSViewVectors;
}

const MSSViewVectors* CMSSRules::GetMSSViewVectors()const
{
	return &g_MSSViewVectors;
}
...
//	g_pLastRebelSpawn = NULL;
	...
	CBaseEntity *pEnt = 
#endif
	CBaseEntity::Create( "MSS_gamerules", vec3_origin, vec3_angle ); // BOXBOX 
	Assert( pEnt );
#endif
}
...
{
#ifndef CLIENT_DLL
	CWeaponMSSBase *pHL2Weapon = dynamic_cast< CWeaponMSSBase*>( pWeapon );
...
CWeaponMSSBase* IsManagedObjectAWeapon( CBaseEntity *pObject )
{
	return dynamic_cast< CWeaponMSSBase*>( pObject );
}
...
bool GetObjectsOriginalParameters( CBaseEntity *pObject, Vector &vOriginalOrigin, QAngle &vOriginalAngles )
{
	...
	else if ( CWeaponMSSBase *pWeapon = IsManagedObjectAWeapon( pObject )) 
	{
...
void CMSSRules::ManageObjectRelocation( void )
{
...
// BOXBOX TODO redo death notice for MSS
...
#ifndef CLIENT_DLL
	void CMSSRules::InitDefaultAIRelationships( void )
	{
		int i, j;
