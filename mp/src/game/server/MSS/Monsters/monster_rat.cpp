...
// BOXBOX The MSS Rat!  Originally based on headcrab code
...
//-----------------------------------------------------------------------------
void CBaseRat::CrawlFromCanister()
{
...
// Input  : NewActivity - 
//-----------------------------------------------------------------------------
void CBaseRat::OnChangeActivity( Activity NewActivity )
{
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CBaseRat::Classify( void )
{
	if( m_bHidden )
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CBaseRat::BodyTarget( const Vector &posSrc, bool bNoisy ) 
{ 
	Vector vecResult;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CBaseRat::GetAutoAimRadius()
{ 
...
// Output : float
//-----------------------------------------------------------------------------
float CBaseRat::MaxYawSpeed( void )
{
...
bool CBaseRat::IsFirmlyOnGround()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseRat::MoveOrigin( const Vector &vecDelta )
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CBaseRat::ThrowAt( const Vector &vecPos )
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CBaseRat::JumpToBurrowHint( CAI_Hint *pHint )
{
...
// Input  : vecVel - 
//-----------------------------------------------------------------------------
void CBaseRat::Leap( const Vector &vecVel )
{
	SetTouch( &CBaseRat::LeapTouch );
...
void CBaseRat::ThrowThink( void )
{
	...
	if( GetFlags() & FL_ONGROUND )
	{
		SetThink( &CBaseRat::CallNPCThink );
...
//			bThrown - 
//-----------------------------------------------------------------------------
void CBaseRat::JumpAttack( bool bRandomJump, const Vector &vecPos, bool bThrown )
{
	Vector vecJumpVel;
	if ( !bRandomJump )
	{
...
// Input  : *pEvent - 
//-----------------------------------------------------------------------------
void CBaseRat::HandleAnimEvent( animevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
void CBaseRat::SetBurrowed( bool bBurrowed )
{
	if ( bBurrowed )
	{
		AddEffects( EF_NODRAW );
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CBaseRat::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CBaseRat::LeapTouch( CBaseEntity *pOther )
{
	m_bMidJump = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CBaseRat::CalcDamageInfo( CTakeDamageInfo *pInfo )
{
...
//-----------------------------------------------------------------------------
void CBaseRat::TouchDamage( CBaseEntity *pOther )
{
	CTakeDamageInfo info;
	CalcDamageInfo( &info );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CBaseRat::GatherConditions( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseRat::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
void CBaseRat::EliminateRollAndPitch()
{
	...
	SetContextThink( &CBaseRat::EliminateRollAndPitch, gpGlobals->curtime + TICK_INTERVAL, s_pPitchContext );
}
...
//-----------------------------------------------------------------------------
void CBaseRat::BeginClimbFromCanister()
{
...
void CBaseRat::JumpFromCanister()
{
	Assert( GetMoveParent() );
...
void CBaseRat::DropFromCeiling( void )
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
void CBaseRat::PlayerHasIlluminatedNPC( CBasePlayer *pPlayer, float flDot )
{
	if ( flDot < 0.97387f )
		return;
...
bool CBaseRat::CanBeAnEnemyOf( CBaseEntity *pEnemy )
{
#ifdef HL2_EPISODIC
	if ( IsHangingFromCeiling() )
		return false;
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CBaseRat::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_HEADCRAB_WAIT_FOR_BARNACLE_KILL:
...
// Output :
//-----------------------------------------------------------------------------
float CBaseRat::InnateRange1MinRange( void )
{
	return HEADCRAB_MIN_JUMP_DIST;
}

float CBaseRat::InnateRange1MaxRange( void )
{
	return HEADCRAB_MAX_JUMP_DIST;
}

int CBaseRat::RangeAttack1Conditions( float flDot, float flDist )
{
...
// Output :
//------------------------------------------------------------------------------
bool CBaseRat::CorpseGib( const CTakeDamageInfo &info )
{
...
// Input  :
//------------------------------------------------------------------------------
void CBaseRat::Touch( CBaseEntity *pOther )
{ 
...
// Output : 
//-----------------------------------------------------------------------------
int CBaseRat::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseRat::ClampRagdollForce( const Vector &vecForceIn, Vector *vecForceOut )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseRat::Event_Killed( const CTakeDamageInfo &info )
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CBaseRat::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseRat::SelectSchedule( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CBaseRat::SelectFailSchedule( int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode )
{
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CBaseRat::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseRat::Ignite( float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner )
{
...
//-----------------------------------------------------------------------------
bool CBaseRat::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType ==	g_interactionVortigauntStompHit)
	{
		...
		CBaseRat *pEntity = (CBaseRat*) CreateEntityByName( "npc_headcrab" );
		pEntity->Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseRat::FValidateHintType( CAI_Hint *pHint )
{
	return true;
}
...
// Input  : &origin - 
//-----------------------------------------------------------------------------
void CBaseRat::ClearBurrowPoint( const Vector &origin )
{
...
//-----------------------------------------------------------------------------
bool CBaseRat::ValidBurrowPoint( const Vector &point )
{
	trace_t	tr;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseRat::GrabHintNode( CAI_Hint *pHint )
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseRat::FindBurrow( const Vector &origin, float distance, bool excludeNear )
{
...
//-----------------------------------------------------------------------------
void CBaseRat::InputUnburrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CBaseRat::InputBurrow( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
//-----------------------------------------------------------------------------
void CBaseRat::InputBurrowImmediate( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseRat::InputStartHangingFromCeiling( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseRat::InputDropFromCeiling( inputdata_t &inputdata )
{
	if ( IsAlive() == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseRat::CreateDust( bool placeDecal )
{
	trace_t	tr;
	AI_TraceLine( GetAbsOrigin()+Vector(0,0,1), GetAbsOrigin()-Vector(0,0,64), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr );
...
///////////////////////////////////////////////////////////////////////////////
// BOXBOX END BASE CLASS
///////////////////////////////////////////////////////////////////////////////

void CRat::Precache( void )
{
	PrecacheModel( "models/monsters/Rat/rat.mdl" );
...
{
	SetMSSLevelModifier( 0 ); // BOXBOX it's the base type of rat

	Precache();
	SetModel( "models/monsters/Rat/rat.mdl" );
...
// Purpose: 
//-----------------------------------------------------------------------------
Activity CRat::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRat::PainSound( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CRat::DeathSound( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CLargeRat::Spawn( void )
{
	SetMSSLevelModifier( 1 ); // BOXBOX Large rats get a bonus level over common rats
...
// Purpose: 
//-----------------------------------------------------------------------------
void CLargeRat::AlertSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CLargeRat::PainSound( const CTakeDamageInfo &info )
{
	if( IsOnFire() && random->RandomInt( 0, HEADCRAB_BURN_SOUND_FREQUENCY ) > 0 )
	{
		// Don't squeak every think when burning.
...
// Purpose: 
//-----------------------------------------------------------------------------
void CLargeRat::DeathSound( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CLargeRat::PrescheduleThink( void )
{
...
// Input  : scheduleType - 
//-----------------------------------------------------------------------------
int	CLargeRat::SelectSchedule( void )
{
...
// Input  : scheduleType - 
//-----------------------------------------------------------------------------
int CLargeRat::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CLargeRat::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_RANGE_ATTACK1:
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CLargeRat::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_HEADCRAB_HOP_ASIDE:
...
LINK_ENTITY_TO_CLASS( monster_rat_large, CLargeRat );
...
// Input  : 
//-----------------------------------------------------------------------------
void CLargeRat::BiteSound( void )
{
	EmitSound( "NPC_FastHeadcrab.Bite" );
}

void CLargeRat::AttackSound( void )
{
...
//-----------------------------------------------------------------------------
void CBaseRat::BuildScheduleTestBits( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
float CLargeRat::MaxYawSpeed( void )
{
	switch ( GetActivity() )
	{
		case ACT_IDLE:
...
bool CLargeRat::QuerySeeEntity(CBaseEntity *pSightEnt, bool bOnlyHateOrFearIfNPC )
{
...
LINK_ENTITY_TO_CLASS( monster_rat_huge, CBossRat );
LINK_ENTITY_TO_CLASS( monster_rat_boss, CBossRat );


//-----------------------------------------------------------------------------
// Purpose: Make the sound of this headcrab chomping a target.
//-----------------------------------------------------------------------------
void CBossRat::BiteSound( void )
{
...
//-----------------------------------------------------------------------------
void CBossRat::AttackSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBossRat::TelegraphSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBossRat::Spawn( void )
{
	SetMSSLevelModifier( 2 ); // BOXBOX 2 bonus levels for boss rats!
...
//-----------------------------------------------------------------------------
float CBossRat::MaxYawSpeed( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
Activity CBossRat::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBossRat::PrescheduleThink( void )
{
	BaseClass::PrescheduleThink();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBossRat::TranslateSchedule( int scheduleType )
{
	switch ( scheduleType )
	{
...
//			In the most cases the base implementation should be called first.
//-----------------------------------------------------------------------------
void CBossRat::BuildScheduleTestBits( void )
{
...
// Output : 
//-----------------------------------------------------------------------------
int CBossRat::SelectSchedule( void )
{
...
//-----------------------------------------------------------------------------
void CBossRat::TouchDamage( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
void CBossRat::Eject( const QAngle &vecAngles, float flVelocityScale, CBaseEntity *pEnemy )
{
	SetGroundEntity( NULL );
...
//-----------------------------------------------------------------------------
void CBossRat::EjectTouch( CBaseEntity *pOther )
{
	LeapTouch( pOther );
	if ( GetFlags() & FL_ONGROUND )
	{
...
//-----------------------------------------------------------------------------
void CBossRat::Panic( float flDuration )
{
...
bool CBossRat::FInViewCone( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
void CBossRat::JumpFlinch( const Vector *pvecDir )
{
	SetGroundEntity( NULL );
...
// Input  : pEvent - 
//-----------------------------------------------------------------------------
void CBossRat::HandleAnimEvent( animevent_t *pEvent )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CBossRat::IsHeavyDamage( const CTakeDamageInfo &info )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBossRat::PainSound( const CTakeDamageInfo &info )
{
	if( IsOnFire() && random->RandomInt( 0, HEADCRAB_BURN_SOUND_FREQUENCY ) > 0 )
	{
		// Don't squeak every think when burning.
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBossRat::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
void CBossRat::ImpactSound( void )
{
...
AI_BEGIN_CUSTOM_NPC( npc_headcrab, CBaseRat )
...
AI_BEGIN_CUSTOM_NPC( npc_headcrab_poison, CBossRat )
...
AI_BEGIN_CUSTOM_NPC( npc_headcrab_fast, CLargeRat )
