...
void Bot_Think( CMSS_Player *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	CMSS_Player *pPlayer = ((CMSS_Player *)CBaseEntity::Instance( pEdict ));
	pPlayer->ClearFlags();
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CMSS_Player *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
