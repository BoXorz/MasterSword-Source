...
class CMSSPlayerMove : public CPlayerMove
{
	DECLARE_CLASS(CMSSPlayerMove, CPlayerMove);
public:
	CMSSPlayerMove() :
...
static CMSSPlayerMove g_PlayerMove;
...
void CMSSPlayerMove::SetupMove(CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *pHelper, CMoveData *move)
{
	// Call the default SetupMove code.
	BaseClass::SetupMove( player, ucmd, pHelper, move );

	// Convert to HL2 data.
	CHL2MP_Player *pMSSPlayer = static_cast<CHL2MP_Player*>( player );
	Assert( pMSSPlayer );

	CMSSMoveData *pHLMove = static_cast<CMSSMoveData*>(move);
	Assert( pHLMove );

	player->m_flForwardMove = ucmd->forwardmove;
	player->m_flSideMove = ucmd->sidemove;

	pHLMove->m_bIsSprinting = pMSSPlayer->IsSprinting();
...
void CMSSPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd, CMoveData *move)
{
	...
	{		
		...
		if ( distance > 0 )
		{
			gamestats->Event_PlayerTraveled( player, distance, pVehicle ? true : false, !pVehicle && static_cast< CHL2MP_Player * >( player )->IsSprinting() );
		}
	}
